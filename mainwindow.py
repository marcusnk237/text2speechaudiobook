...
from pdf2audio import text2mp3
...
class MainWindow(QMainWindow):
    ...
    def exportFiles(self, file_name=''):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, "Audio output", default_filename, "Audio Files (*.mp3)"
            )
        if filename:
            text2mp3 (file_path , filename ,'fr')
            self.succespopup()

    def succespopup(self):
        msgBox = QMessageBox()
        msgBox.setText("Audio file successfully saved.")
        msgBox.exec()
    ...
    def open(self, doc_location):
        ...
        if doc_location.isLocalFile():
            ...
            self.setWindowTitle(document_title if document_title else "PDF to Audio")
    ...
    def on_actionAbout_triggered(self):
        QMessageBox.about(self, "About PdftoAudio",
                          "A GUI which convert a PDF to a mp3 file")
    ...
    @Slot()
    def on_actionConvert_triggered(self):
        self.exportFiles()
